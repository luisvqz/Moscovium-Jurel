...
setwd("destiny") #replace destiny with the folder containing the phyloseq_req folder
...
bandb1<-prune_samples(sample_sums(bandb)>=8000,bandb)
bandb1
### RAREFACTION
bandb<-rarefy_even_depth(bandb1, sample.size = min(sample_sums(bandb1)))
bandb
...
family_species_loc_type_coquimatlan <- ggplot(join_Family_loc_type_coquimatlan, aes(x = Type2, y = rel.freq, fill = Family)) + 
  geom_bar(stat = "identity", position = "fill") +
  ylab("Relative abundance") +
  xlab("Type2") +ggtitle("composition by sample type in Coquimatl?n")+ scale_fill_manual(values=Taxacolors)+ compositiontheme
...
Family_species_loc_type_batfly_coquimatlan <- ggplot(join_Family_loc_type_batfly_coquimatlan, aes(x = Type2, y = rel.freq, fill = Family)) + 
  geom_bar(stat = "identity", position = "fill") +
  ylab("Relative abundance") +
  xlab("Type2") +ggtitle("composition by batfly Family in Coquimatl?n")+ scale_fill_manual(values=Taxacolors)+ compositiontheme
...
Genus_species_loc_type_batfly_coquimatlan <- ggplot(join_Genus_loc_type_batfly_coquimatlan, aes(x = Type2, y = rel.freq, fill = Genus)) + 
  geom_bar(stat = "identity", position = "fill") +
  ylab("Relative abundance") +
  xlab("Type2") +ggtitle("composition by batfly Genus in Coquimatl?n")+ scale_fill_manual(values=genuscolors)+ compositiontheme
...
bandbCoquimatlan<- bandb1%>%
  subset_samples(Locality=="Coquimatl?n")
...
#plot ordination for Coquimatl?n
...
W_unifrac_plot_by_Type2_coquimatlan <- ggplot(data = centroids_W_coquimatlan, aes(x=mean_MDS1, y=mean_MDS2, color=Type2))+
  ...
  ggtitle("Weighted UniFrac Coquimatl?n") +themebetaplots
...
UW_unifrac_plot_by_Type2_coquimatlan <- ggplot(data = centroids_UW_coquimatlan, aes(x=mean_MDS1, y=mean_MDS2, color=Type2))+
  ...
  ggtitle("UnWeighted UniFrac Coquimatl?n") +themebetaplots
